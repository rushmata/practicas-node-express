...
module.exports = {
      ...
      if (err) {
        ...
        for (let videogame of videogames) {
          videogamesList.push({
            id: videogame._id,
            name: videogame.name,
            released_on: videogame.released_on,
          });
        }
        res.json({
          status: "success",
          message: "Videogames list found!!!",
          data: { videogames: videogamesList },
        });
      }
    });
  },
  ...
  deleteById: function (req, res, next) {
    videogameModel.findByIdAndRemove(
      req.params.videogameId,
      function (err, videogameInfo) {
        if (err) next(err);
        else {
